...
module @jit_concatenate attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<4x1024x2048xbf16> loc("args[0]"), %arg1: tensor<4x128x2048xbf16> loc("args[1]"), %arg2: tensor<4x128x2048xbf16> loc("args[2]")) -> (tensor<4x1280x2048xbf16> {jax.result_info = "result"}) {
    %0 = stablehlo.concatenate %arg0, %arg1, %arg2, dim = 1 : (tensor<4x1024x2048xbf16>, tensor<4x128x2048xbf16>, tensor<4x128x2048xbf16>) -> tensor<4x1280x2048xbf16> loc(#loc33)
    return %0 : tensor<4x1280x2048xbf16> loc(#loc)
  } loc(#loc)
} loc(#loc)
#loc = loc(unknown)
