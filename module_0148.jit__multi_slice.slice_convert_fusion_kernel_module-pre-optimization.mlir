module @slice_convert_fusion_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @slice_convert_fusion(%arg0: tensor<5120x2048xbf16> {llvm.align = 64 : index, llvm.dereferenceable = 20971520 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<1280x2048xbf16> {llvm.align = 64 : index, llvm.dereferenceable = 5242880 : index, xla.slice_index = 1 : index}) -> tensor<1280x2048xbf16> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %3 = scf.forall (%arg2, %arg3, %arg4) in (1, 1, 1) shared_outs(%arg5 = %arg1) -> (tensor<1280x2048xbf16>) {
      %xla_loop = xla.loop (%arg2, %arg3, %arg4, %0, %1, %2)[%i, %j] -> (%ra, %rb) in #xla.indexing_map<"(th_x, th_y, th_z, bl_x, bl_y, bl_z)[s0, s1] -> (bl_x * 92 + s0, s1), domain: th_x in [0, 0], th_y in [0, 0], th_z in [0, 0], bl_x in [0, 13], bl_y in [0, 0], bl_z in [0, 0], s0 in [0, 91], s1 in [0, 2047], bl_x * 92 + s0 in [0, 1279]"> iter_args(%iter = %arg5) -> (tensor<1280x2048xbf16>) {
        %pure_call = xla.pure_call @fused_computation_convert_8(%arg0, %ra, %rb) : (tensor<5120x2048xbf16>, index, index) -> bf16
  ...
  func.func private @fused_computation_convert_8(%arg0: tensor<5120x2048xbf16>, %arg1: index {xla.range = [0 : index, 1279 : index]}, %arg2: index {xla.range = [0 : index, 2047 : index]}) -> bf16 attributes {llvm.linkage = #llvm.linkage<internal>} {
    %0 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d0 + 3840), domain: d0 in [0, 1279], d1 in [0, 2047]">(%arg1, %arg2)
