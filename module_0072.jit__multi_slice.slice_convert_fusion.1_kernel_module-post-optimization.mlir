  ...
  func.func @slice_convert_fusion.1(%arg0: tensor<4958208xbf16> {llvm.align = 64 : index, llvm.dereferenceable = 9916416 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<1239552xbf16> {llvm.align = 64 : index, llvm.dereferenceable = 2479104 : index, xla.slice_index = 1 : index}) -> tensor<1239552xbf16> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %2 = scf.if %1 -> (tensor<1239552xbf16>) {
      ...
      %5 = scf.if %4 -> (tensor<1239552xbf16>) {
        %6 = scf.for %arg2 = %c0 to %c77 step %c1 iter_args(%arg3 = %arg1) -> (tensor<1239552xbf16>) {
          %7 = scf.for %arg4 = %c0 to %c1152 step %c1 iter_args(%arg5 = %arg3) -> (tensor<1239552xbf16>) {
            %8 = xla.apply_indexing #xla.indexing_map<"(d0, bl_x, d2) -> (bl_x * 88704 + d2 * 1152 + d0 + 2479104), domain: d0 in [0, 1151], bl_x in [0, 12], d2 in [0, 76]">(%arg4, %0, %arg2)
            %extracted = tensor.extract %arg0[%8] : tensor<4958208xbf16>
            %9 = xla.apply_indexing #xla.indexing_map<"(d0, bl_x, d2) -> (bl_x * 88704 + d2 * 1152 + d0), domain: d0 in [0, 1151], bl_x in [0, 12], d2 in [0, 76]">(%arg4, %0, %arg2)
      ...
      %4 = scf.if %3 -> (tensor<1239552xbf16>) {
        %5 = scf.for %arg2 = %c0 to %c75 step %c1 iter_args(%arg3 = %arg1) -> (tensor<1239552xbf16>) {
          %6 = scf.for %arg4 = %c0 to %c1152 step %c1 iter_args(%arg5 = %arg3) -> (tensor<1239552xbf16>) {
            %7 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d1 * 1152 + d0 + 3632256), domain: d0 in [0, 1151], d1 in [0, 74]">(%arg4, %arg2)
            %extracted = tensor.extract %arg0[%7] : tensor<4958208xbf16>
            %8 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d1 * 1152 + d0 + 1153152), domain: d0 in [0, 1151], d1 in [0, 74]">(%arg4, %arg2)
